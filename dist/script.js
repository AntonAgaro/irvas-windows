...
//Работаем с объектом(let modalState = {};), запоминающим данные из формы - калькулятора
...
var changeModalState = function changeModalState(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height'); //Функция, вешающая обрадотчик на элемент и записывающая значение в объект state
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); //Создаем блок со статусом сообщения отправки формы и помещаем его в конец перебираемой формы

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.appendChild(statusMessage); // Собираем данные из формы при помощи объекта formData
...
var timer = function timer(id, deadline) {
  //Функиция для указания 0 во времени
  ...
  var getTimeRemaining = function getTimeRemaining(endtime) {
    //Время разница между дедлайном и текущей датой
